...
def main(lensname,dataname,work_dir='./'):
    ...
    errors_up_list = []
    errors_down_list = []
    simset_mock_ava = ["mocks_n%it%i_%s" % (int(config.nsim * config.nsimpkls), config.truetsr,i) for i in config.tweakml_name_marg_regdiff]
    opt = 'regdiff'
    ...
    print "Final combination for marginalisation ", config.name_marg_regdiff
    combined.niceprint()
    ...
    pycs.mltd.plot.delayplot(group_list+[combined], rplot=8.0, refgroup=combined, text=text, hidedetails=True, showbias=False, showran=False, showlegend=True, figsize=(15, 10), horizontaldisplay=False, legendfromrefgroup=False, filename = indiv_marg_dir + config.name_marg_regdiff +"_sigma_%2.2f.png"%config.sigmathresh)
