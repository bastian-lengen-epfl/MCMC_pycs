...
def write_report_checkstat(f, lcs, stats, combkw, sset, ooset,  tolerance = 1.0):
    ...
    if all(success) :
        f.write("Sucessfully matched zruns and sigmas within %2.2f sigmas \n"%tolerance)
    else :
            ...
            for a in simset_available :
                ...
                if "_opt_" in a : # take only the optimised sub-folders
                    ...
                    if ooset[0:7] == 'regdiff' :
                        ...
                        stats = pycs.gen.stat.anaoptdrawn(lcs, spline, simset=sset, optset=ooset, showplot=False, nplots= 1,
