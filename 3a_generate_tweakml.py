...
def run_MCMC(lcs,spline,kn,optim_directory, config_file,stream):
    config = importlib.import_module(config_file)
    fit_vector = mcmc.get_fit_vector(lcs, spline)
    pycs.sim.draw.saveresiduals(lcs, spline)
...
def run_DIC(lcs,spline,kn,optim_directory, config_file,stream):
    config = importlib.import_module(config_file)
    pycs.sim.draw.saveresiduals(lcs, spline)
    fit_vector = mcmc.get_fit_vector(lcs, spline)
...
def main(lensname,dataname,work_dir='./'):
    ...
    for i,kn in enumerate(config.knotstep):
        for j, knml in enumerate(config.mlknotsteps):
            ...
            lcs, spline = pycs.gen.util.readpickle(config.lens_directory + '%s/initopt_%s_ks%i_ksml%i.pkl' % (config.combkw[i, j], dataname, kn, knml))
            ...
            if config.tweakml_type == 'colored_noise':
                ...
                if config.find_tweak_ml_param == True :
                    if config.optimiser == 'PSO':
                        run_PSO(lcs,spline,kn,optim_directory,config_file,f)
                    elif config.optimiser == 'MCMC' :
                        run_MCMC(lcs, spline, kn, optim_directory,config_file,f)
                    else :
                ...
                if config.find_tweak_ml_param == True :
                    if config.optimiser == 'PSO':
                        ...
                        run_DIC(lcs, spline, kn, optim_directory,config_file,f)
                    else :
